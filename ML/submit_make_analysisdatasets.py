...
keeplist = ["TT","TTZ","TTH","TTG", "TTW"]


years = ['2016PreVFP','2016PostVFP','2017','2018']

originalfolder = "/pnfs/iihe/cms/store/user/nivanden/skims_v4/{}/"
storefolder = "/pnfs/iihe/cms/store/user/dmarckx/ttWsamples/"

for year in years:
  commands = []
  copycommands = []
  inputdir = originalfolder.format(year)
  samplelist = "/user/dmarckx/ewkino/ttWAnalysis/samplelists/fourtops_notused/samples_tttt_{}_sim.txt".format(year)
  my_file = open(samplelist, "r")
  ...
  data_into_list = [x for x in data_into_list if not x.startswith("#")]

          
  for i in range(len(data_into_list)):
      data_into_list[i] = data_into_list[i].split("  ")
  name_list = [z[1] for z in data_into_list]
  type_list = [z[0] for z in data_into_list]
  xsec_list = [z[2] for z in data_into_list]

  my_file.close()

  for i in range(len(name_list)):
      if type_list[i] not in keeplist:
          # copy the whole file in the analysis directory
          print("just copying " + name_list[i])
          if not os.path.exists(storefolder + year + "/" + name_list[i]):
            cmd = "cp {} {}".format(originalfolder.format(year) + name_list[i], storefolder + year + "/" + name_list[i]) 
            copycommands.append(cmd)
            print(type_list[i])

      else:
          # make a submission to skim this file
          print("run over: " + type_list[i])
          #cmd = "python3 make_analysisdatasets.py {} {} {} {} {} {} {}".format(name_list[i], type_list[i], i, year, samplelist, inputdir, storefolder + year + "/")
          #commands.append(cmd)
  #print(commands)
  if commands != []:
    submitCommandsAsCondorCluster('cjob_makeanalysis', commands, stdout=None, stderr=None, log=None,
                         ...
                         jobflavour=None)
  if copycommands != []:
    submitCommandsAsCondorCluster('cjob_makeanalysis_copies', copycommands, stdout=None, stderr=None, log=None,
                         cpus=4, mem=2048, disk=10240,
                         home=None,
                         proxy=None,
                         cmssw_version="CMSSW_12_4_6",
