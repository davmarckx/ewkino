...
for train_index, test_index in cv.split(X, y):
    ...
    fpr, tpr, thresholds = roc_curve(y_test2.astype(int).to_numpy(), y_score.astype(float), sample_weight = weight_test2.astype(float))
    fpr_other, tpr_other, tresholds = roc_curve(y_other2.astype(int).to_numpy(), y_scoreother.astype(float),sample_weight =weight_other2.astype(float))
    ...
    aucs_other.append(roc_auc_score(y_other2.astype(int), y_scoreother.astype(float)))
    i += 1
...
plt.savefig("/user/dmarckx/public_html/ML/BDT/kfoldROC_robustnessv3_{}_".format(year) + "_" + str(len(X_train.columns)) + "_" + str(n_estimators) + "_" + str(max_depth) + "_" + str(lr) + dt_string + ".png")
