...
#################################
# make 1D likelihood scan plots #
#################################
# documentation: https://cms-analysis.github.io/HiggsAnalysis-CombinedLimit/part3/commonstatsmethods/#likelihood-fits-and-scans
# this script was copied from CombineHarvester/CombineTools/scripts/plot1DScan.py
# with local modifications to allow repositioning of legends etc.
...
parser.add_argument('--main_label', default='Observed', type=str, help='legend label for the main scan')
parser.add_argument('--main_color', default=1, type=int, help='line and marker color for main scan')
...
# define text with central values and uncertainties
textfit = '#color[%s]{%s = %.3f{}^{#plus %.3f}_{#minus %.3f}}' % (
  args.main_color, fixed_name, val_nom[0], val_nom[1], abs(val_nom[2]))
pt = ROOT.TPaveText(0.75, 0.8 - (len(other_scans)+1)*0.04, 0.95, 0.8, 'NDCNB')
pt.AddText(textfit)
