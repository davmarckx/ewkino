...
def plotdifferential_compar(
    ...
    ### style operations on data histogram
    datahist1.SetMarkerStyle(markerstyle)
    datahist1.SetMarkerColor(markercolor)
    datahist1.SetMarkerSize(markersize)
    datahist1.SetLineColor(markercolor)
    statdatahist1.SetMarkerSize(0)
    statdatahist1.SetLineColor(ROOT.kRed)

    datahist2.SetMarkerStyle(markerstyle2)
    datahist2.SetMarkerColor(markercolor2)
    datahist2.SetMarkerSize(markersize)
    datahist2.SetLineColor(markercolor2)
    statdatahist2.SetMarkerSize(0)
    statdatahist2.SetLineColor(46)

    ### make ratio histograms
    ratiohistlist = [h.Clone() for h in theoryhists]
    ratiodatahist = datahist1.Clone()
    ratiostatdatahist = statdatahist1.Clone()
    allratiohists = ratiohistlist+[ratiodatahist]+[ratiostatdatahist]
    ratiosysthists = None
    if systhists is not None:
        ratiosysthists = []
    ...
    for hist in allratiohists:
        for j in range(0,hist.GetNbinsX()+2):
            scale = datahist1.GetBinContent(j)
            if scale<1e-12:
                hist.SetBinContent(j,0)
                hist.SetBinError(j,10)
            else:
    ...
    legend.SetTextFont(10*legendfont+3)
    legend.SetBorderSize(1)
    legend.AddEntry(datahist1,'Fit result',"pe1")
    legend.AddEntry(datahist2,'Old fit result',"pe1")
    for i,hist in enumerate(theoryhists):
        label = hist.GetTitle()
        if labellist is not None: label = labellist[i]
        legend.AddEntry(hist,label,"l")
    ...
    # get x-limits (for later use)
    nbins = datahist1.GetNbinsX()
    xlims = (datahist1.GetBinLowEdge(1),
             datahist1.GetBinLowEdge(nbins)+datahist1.GetBinWidth(nbins))
    # get and set y-limits
    (totmin,totmax) = ht.getminmax(theoryhists+[datahist1])
    # in case of log scale
    if logy:
        pad1.SetLogy()
        if ylims is None: ylims = (totmin*yminlogfactor, totmax*ymaxlogfactor)
    ...
    x_shift = (datahist1.GetBinLowEdge(nbins)+datahist1.GetBinWidth(nbins) - datahist1.GetBinLowEdge(1)) / 50
    
    xbins1 = datahist1.GetXaxis().GetXbins()
    xbins2 = datahist2.GetXaxis().GetXbins()
    for i in range(len(xbins1)):
      xbins1[i] = xbins1[i] - x_shift
      xbins2[i] = xbins2[i] + x_shift


    datahist1.GetXaxis().Set((xbins1.GetSize() - 1),xbins1.GetArray())
    statdatahist1.GetXaxis().Set((xbins1.GetSize() - 1),xbins1.GetArray())

    datahist2.GetXaxis().Set((xbins2.GetSize() - 1),xbins2.GetArray())
    statdatahist2.GetXaxis().Set((xbins2.GetSize() - 1),xbins2.GetArray())


    datahist1.Draw("pe e1 x0 same")
    statdatahist1.Draw("pe e1 x0 same")
    datahist2.Draw("pe e1 x0 same")
    statdatahist2.Draw("pe e1 x0 same")
    ...
    c1.SaveAs(figname.replace('.png','')+'v2.png')
    #c1.SaveAs(figname.replace('.png','')+'.eps')
    #c1.SaveAs(figname.replace('.png','')+'.pdf')
