...
for train_index, test_index in cv.split(X_train_norm, y_train):
    ...
    sNN = TrainNN(X_train_norm2, y_train2, X_test_norm2, y_test2,weight_train2,weight_test2, classweight, dropval,learr,beta1, beta2, batchsize,"kfold/kfoldNN_" + str(i), epochs=epochs, manualNjobs=8)
...
figu.savefig("/user/dmarckx/public_html/ML/NN/ROCs/kfoldROCNN_" + str(fract) + "drop" + str(int(dropval*100)) + "_" + str(epochs) + "_" + str(learr) + "_(" + str(int(beta1*10)) + "," + str(int(beta2*10)) + ")" + str(batchsize) + ".png")
plt.close(figu) 
