...
void fillSystematicsHistograms(
    ...
    // (hard coded for now!!!) 
    else if(treeReader.isData()){bTagShapeSystematics = {"cferr1", "cferr2", "hf",
    ...
    // (hard coded for now!!!)
        ...
        && (considerjecgrouped)
        && treeReader.isData() ){
        groupedJECVariations = {"Absolute_" + year.substr(0, 4),"Absolute",
          "BBEC1_"+year.substr(0, 4),"BBEC1","EC2_"+year.substr(0, 4),"EC2",
          "FlavorQCD","HF_"+year.substr(0, 4),"HF","RelativeBal",
	...
	if( treeReader.isData() && !(st=="fakerate" || st=="efakerate" || st=="mfakerate") ) continue;
...
int main( int argc, char* argv[] ){
    ...
    fillSystematicsHistograms( input_directory, sample_list, sample_index, output_directory,
			       ...
			       bdtWeightsFile, bdtCutValue, trainingreweight, systematics );
