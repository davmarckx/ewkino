...
years = ['all']#,'2016PreVFP','2016PostVFP','2017','2018']
dropval = 0.0 
regdropval =  0.2
...
epochs = [20,20,20]
nr_events = -1
sparse = ['sparse','fullyconnected']          #sparse,fullyconnected or loose
njobs = 8

for year in years:
    commands = []
    for i in range(len(sparse)):
        commands.append("python3.9 trainMCGNN.py " + str(dropval) + " " + str(regdropval) + " " + str(learr[i]) + " " + str(beta1) + " " + str(beta2) + " " + str(batchsize) + " " + str(epochs[i]) + " " + str(nr_events) + " " + str(year) + " " + sparse[i] + " " + str(njobs))
    submitCommandsAsCondorCluster('cjob_train_MC_GNN', commands, stdout=None, stderr=None, log=None,
                        cpus=njobs, mem=6144, disk=10240,
                        home=None,
                        proxy=None,
